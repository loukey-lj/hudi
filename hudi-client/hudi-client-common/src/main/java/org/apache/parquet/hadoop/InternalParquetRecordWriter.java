...
class InternalParquetRecordWriter<T> {
   ...
   * @return
   */
  public ParquetFileWriter getParquetFileWriter() {
  ...
  public void close(Map<String, String> footerMap) throws IOException, InterruptedException {
    if (!closed) {
      flushRowGroupToStore();
      FinalizedWriteContext finalWriteContext = new WriteSupport.FinalizedWriteContext(footerMap);
      Map<String, String> finalMetadata = new HashMap<String, String>(extraMetaData);
      String modelName = writeSupport.getName();
      if (modelName != null) {
        finalMetadata.put(ParquetWriter.OBJECT_MODEL_NAME_PROP, modelName);
  ...
  private void flushRowGroupToStore()
    ...
    LOG.info("Flushing mem columnStore to file. allocated memory: {}, recordCount {} ", columnStore.getAllocatedSize(), recordCount);
