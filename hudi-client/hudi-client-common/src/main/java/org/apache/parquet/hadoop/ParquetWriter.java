...
public class ParquetWriter<T> implements Closeable {
   ...
   * @return
   */
  public ParquetFileWriter getOrginialParquetFileWriter() {
     return writer.getParquetFileWriter();
  }
  ...
  public void close(Map<String, String> footerMap) throws IOException {
    try {
      writer.close(footerMap);
    } catch (InterruptedException e) {
      throw new IOException(e);
    } finally {
      // release after the writer closes in case it is used for a last flush
